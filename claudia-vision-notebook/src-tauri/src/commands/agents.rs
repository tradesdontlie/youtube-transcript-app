...
use crate::commands::migrations::run_migrations;
...
pub fn init_database(app: &AppHandle) -> SqliteResult<Connection> {
    ...
    // Create transcript_history table for YouTube transcript app
    conn.execute(
    ...
    // Create notebook_entries table for general notebook entries
    conn.execute(
        "CREATE TABLE IF NOT EXISTS notebook_entries (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            entry_type TEXT NOT NULL, -- 'claude_session', 'transcript', 'note', 'project'
            entry_id INTEGER, -- Foreign key to specific entry type
            title TEXT NOT NULL,
